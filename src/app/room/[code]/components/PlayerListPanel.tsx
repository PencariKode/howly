...
function GamePlayerCard({ player, isCurrentUser, forceShowRole, onKillPlayer }: {
    ...
    return (
        <div className={ `flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg border transition-all duration-200 ${!player.isAlive
            ...
            <div className="shrink-0">
                { showRole && roleInfo ? (
                    <span className={ `inline-flex items-center gap-1 px-2 py-1 rounded-md text-[0.65rem] font-semibold border ${roleInfo.side === 'werewolf'
                    ...
                    <span className="inline-flex items-center px-2 py-1 rounded-md text-[0.65rem] font-semibold bg-zinc-800/60 border border-zinc-700/30 text-zinc-500">
                        ???
                    </span>
                ) }
...
export default function PlayerListPanel({ players, isOwner, currentUserId, owner, onKillPlayer }: PlayerListPanelProps) {
